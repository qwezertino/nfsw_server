...
INSERT INTO PARAMETER (\`NAME\`, \`VALUE\`) VALUES ('MODDING_BASE_PATH', 'http://${NEW_IP}:8000')
    ON DUPLICATE KEY UPDATE \`VALUE\` = 'http://${NEW_IP}:8000';
INSERT INTO PARAMETER (\`NAME\`, \`VALUE\`) VALUES ('MODDING_FEATURES', '')
    ON DUPLICATE KEY UPDATE \`VALUE\` = '';
INSERT INTO PARAMETER (\`NAME\`, \`VALUE\`) VALUES ('MODDING_SERVER_ID', 'sbrw-private')
    ON DUPLICATE KEY UPDATE \`VALUE\` = 'sbrw-private';
SQL
...
echo "  bash /home/qwezert/nfsw_server/start.sh --stop"
echo "  bash /home/qwezert/nfsw_server/start.sh"
